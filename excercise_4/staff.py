...
def init_manager(config_file_name, worker_name_list, total_simulation_time):
	...
	simulation_time_for_one_federate = total_simulation_time / len(worker_name_list)
	
	currenttime = -1
	h.helicsFederateEnterExecutingMode(fed)

	h.helicsEndpointSendMessageRaw(log_id, log_dest, str(simulation_time_for_one_federate))

	for worker_name in worker_name_list:
		h.helicsEndpointSendMessageRaw(my_epid, worker_name + "/data", str(simulation_time_for_one_federate))
